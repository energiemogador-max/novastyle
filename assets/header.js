 ...
 * Layout: [Logo + Store Name] [Nav centered] [Cart] [Hamburger]
...
(function () {
  ...
  var css =
    ...
    '@media(max-width:900px){.brand-tagline{display:none;}.brand{margin-right:16px;}.nav-link{padding:6px 8px;font-size:12px;}}' +
    '@media(max-width:768px){' +
    '.hamburger{display:flex;}' +
    '.main-nav{position:fixed;top:0;left:-290px;height:100vh;width:270px;background:#fff;flex-direction:column;align-items:stretch;gap:0;padding:72px 0 32px;z-index:499;box-shadow:6px 0 24px rgba(0,0,0,.12);transition:left .28s cubic-bezier(.4,0,.2,1);overflow-y:auto;}' +
  ...
  function inject() {
    ...
    function openMenu() { nav.classList.add('open'); overlay.classList.add('open'); toggle.classList.add('open'); toggle.setAttribute('aria-expanded','true'); document.body.style.overflow = 'hidden'; }
    function closeMenu() { nav.classList.remove('open'); overlay.classList.remove('open'); toggle.classList.remove('open'); toggle.setAttribute('aria-expanded','false'); document.body.style.overflow = ''; }
